...
const CHUNK = 100;
// PostgREST defaults to a 1000-row cap per request on Supabase. We page in
// 1000-row windows to drain every matching row. Anything smaller burns
// extra round-trips; anything larger hits the cap and silently truncates.
...
/**
 * Drain every row matched by a Supabase query that supports `.range()`.
 * Use this instead of a raw `await query` whenever the result set could
 * exceed PostgREST's 1000-row cap (anything with `.in()` on a wide list,
 * any unbounded global query, anything joined to a many-side table).
 *
 * The query builder must be regenerated by `buildQuery` on each iteration
 * because PostgrestFilterBuilder is consumed by `await` — we need a fresh
 * one for each range window.
 */
...
export async function fetchAnnualizedContractValuesByCustomerId(
  ...
  //
  // Customers are 1 row per id, so even a chunk of 1000 ids fits well under
  // the PostgREST cap. drainPages is still used as defence-in-depth in case
  // chunkArray ever yields a chunk that would somehow exceed the cap.
  // ---------------------------------------------------------------------------
  const fortnoxByCustomerId = new Map<string, string>();
  for (const chunk of chunkArray(uniqueIds, CHUNK)) {
    const { rows, error } = await drainPages<CustomerIdRow>(() =>
      supabase
        .from("customers")
  ...
  // customer number.
  //
  // A chunk of 100 fortnox numbers can easily exceed 1000 contracts in total
  // (5–10 contracts per customer is typical). Pagination is MANDATORY here;
  // without it, the rollup silently undercounts and the top-N ranking by
  // contract value becomes nonsensical (small customers bubble into the
  // top, large multi-contract customers sink).
  // ---------------------------------------------------------------------------
