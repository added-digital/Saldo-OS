-- 00116: keep Saldo's own company off the beläggning board.
--
-- Saldo Redovisning Stockholm AB (Fortnox customer number 1) is the firm's own
-- internal-time account: admin, sälj, möten, intern utveckling. It showed up on
-- the board as a customer needing 741,2 h in August 2026 — roughly 4,4 FTE and
-- 32 % of the entire firm's planned load — sitting in "Behöver bedömning".
--
-- The rule already existed and was already documented as living in one place:
-- src/lib/reports/constants.ts, INTERNAL_FORTNOX_CUSTOMER_NUMBERS. The reports
-- page and the KPI generator both honour it; resource_board was written without
-- it and silently disagreed with every other view in the app.
--
-- Rather than repeat the literal '1' in SQL and let the two copies drift, the
-- fact moves onto the row itself. constants.ts stays the source for the TS
-- paths; customers.is_internal is its mirror for anything running in the
-- database. Adding a second internal account means setting the flag AND adding
-- the number to that constant.

ALTER TABLE customers
  ADD COLUMN IF NOT EXISTS is_internal BOOLEAN NOT NULL DEFAULT false;

COMMENT ON COLUMN customers.is_internal IS
  'Saldo''s own internal-time account rather than a real customer. Mirrors INTERNAL_FORTNOX_CUSTOMER_NUMBERS in src/lib/reports/constants.ts. Excluded from beläggning.';

UPDATE customers
   SET is_internal = true
 WHERE btrim(COALESCE(fortnox_customer_number, '')) = '1'
...
-- resource_board reissued from 00115 with the exclusion in `scope`.
